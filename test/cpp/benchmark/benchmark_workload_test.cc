...
int main() {
  {
    const auto normalized =
        telepath::benchmark_support::NormalizeWorkloadName("sequential");
...

  {
    ...
    for (std::size_t op = 0; op < 16; ++op) {
      const auto first =
          telepath::benchmark_support::ChooseBlock(&rng, op, 0, options);
      const auto second =
          telepath::benchmark_support::ChooseBlock(&rng, op, 3, options);
      assert(first == second);
    }
  }

  {
    ...
    for (std::size_t op = 0; op < 16; ++op) {
      ...
      for (std::size_t thread = 0; thread < options.thread_count; ++thread) {
        blocks.insert(
            telepath::benchmark_support::ChooseBlock(&rng, op, thread, options));
      }
